 ...
 */
template <Numeric OutputType> class Sensor {
...
class ADT7310 : public Sensor<uint16_t> {
  public:
    enum class ADT7310Mode {
        MODE13BITS, /**< Sensor is used in 13 bits mode */
        MODE16BITS  /**< Sensor is used in 16 bits mode */
    };

  private:
    ADT7310Mode _Mode{ADT7310Mode::MODE13BITS}; /**< Sensor mode (13/16 bits) */
  public:
    ADT7310() = delete; /**< Default Ctor is deleted */
    explicit ADT7310(uint32_t uid)
        : Sensor<uint16_t>(uid, "LSB 0.0625°C (Signed 13 bits mode) or LSB 0.0078°C (Signed 16 bits mode)") {};
